...
class SpecialToken:
    ...
    @staticmethod
    def compute_time_gaps(timestamps: pd.Series, unit: str = "minutes") -> pd.Series:
        """
        Computes exact time gaps between consecutive events.

        Args:
            timestamps (pd.Series): Series of datetime values.
            unit (str): Unit for time difference. Options: 'seconds', 'minutes', 'hours', 'days'.

        Returns:
            pd.Series: Time gaps between consecutive events in the specified unit.
        """
        timestamps = pd.to_datetime(timestamps).sort_values()
        time_diffs = timestamps.diff().dt.total_seconds()  # Convert to seconds
        ...
        if unit == "minutes":
            ...
            return time_diffs / 86400
        else:
            return time_diffs  # Default: seconds
